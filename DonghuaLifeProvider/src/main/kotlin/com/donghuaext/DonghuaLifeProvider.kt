...
class DonghuaLifeProvider : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        "$mainUrl/" to "Inicio",
        "$mainUrl/genre/donghua/" to "Donghuas",
        "$mainUrl/genre/anime/" to "Animes"
    )

    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val url = if (page > 1) "${request.data}page/$page/" else request.data
        val doc = app.get(url).document
        val items = doc.select("div.items article, div.post-item, div.bs").mapNotNull { it.toSearchResult() }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.items article, div.post-item, div.bs").mapNotNull { it.toSearchResult() }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = doc.selectFirst("div.bigcontent img, div.thumb img, div.post-thumbnail img")?.attr("abs:src")
        val synopsis = doc.selectFirst("div.entry-content, div.sinopsis, div.desc, div.summary")?.text()?.trim()
        val genres = doc.select("div.genxed a, div.sigen a, a[rel=tag], span.genre a").map { it.text().trim() }

        val episodes = doc.select("div.eplister ul li a, div.lsch a, div.episodelist a, div.listing a").mapNotNull { ep ->
    ...
    override suspend fun loadLinks(
        ...
        // Check for direct video/m3u8 in script tags
